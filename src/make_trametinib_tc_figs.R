...
df <- Embeddings(object = seuObj, reduction = 'tsne') %>% 
  ...
  dplyr::mutate(hash_tag = ifelse(grepl("DMSO", hash_tag), 'control', hash_tag),
...
ggsave('~/CPDS/ApolloSeq/results/figures/trametinib_tc_fulltsne.png',
       width = 4.5, height = 4)
...
}) %>% mutate(condition = str_match(condition, '(Tram_[0-9]+)hr')[,2],
...
get_condition_GSEA <- function(df, gsc, n_top_genes) {
  ...
  ldply(unique(df$condition), function(cur_cond) {
    ...
    mutate(gset_dir = paste0(gene_set, '.', direction),
